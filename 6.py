...
def find_all_orfs(dna_sequence, min_length):
    ...
    filtered_orfs = [orf for orf in all_orfs if len(orf) >= min_length]
...
def main(fasta_file, output_file, min_length):
    with open(output_file, 'a') as out_f:  # Open output file in append mode
        for record in SeqIO.parse(fasta_file, "fasta"):
            ...
            orfs = find_all_orfs(dna_sequence, min_length)
            for protein in orfs:
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python 6.py <min_length> <upstream_range> <input_fasta1> <input_fasta2> ...")
        sys.exit(1)

    min_length = int(sys.argv[1])  # Minimum length for ORF
    upstream_range = int(sys.argv[2])  # Upstream range for RBS

    fasta_files = sys.argv[3:]  # All other arguments are fasta files
    output_file = "output2_genes.txt"  # Specify your desired output file name
